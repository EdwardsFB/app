// version: v1.0.38 | build: 2026-08-03T15:23:07Z
// ══════════════════════════════════════════
...
// Sorts products by their saved sortOrder (set by admin's drag/reorder feature).
// Products missing a sortOrder value (never explicitly reordered) sort to the end,
// keeping their existing relative order via JS's stable sort - not left in
// unpredictable order. Used everywhere products get loaded, so the customer menu
// and admin's own product list both reflect whatever order was actually set.
...
function sourceIcon(source) {
  ...
  if (source === 'admin-manual') return '<i class="bi bi-clipboard-check" title="Entered manually in admin"></i>';
  if (source === 'test') return '<i class="bi bi-bug" title="Test order (ZZTest tool)"></i>';
